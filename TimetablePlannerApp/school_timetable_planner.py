...
class TimetableApp:
    ...
    def export_impacted_classes(self, teacher, day):
            ...
            for section in sections:
                for p in range(periods):
                    ...
                    if teacher_var.get() == teacher:
                        ...
                        # Set Entry widget background to strong red and white text for visibility
                        if hasattr(self, 'teacher_entries') and key in self.teacher_entries:
                            self.teacher_entries[key].config(bg='#ff3333', fg='white', insertbackground='white', highlightthickness=3, highlightbackground='#b20000')
        ...
        row = 0
        teacher_entries = []
        teacher_lower = teacher.strip().lower()
        for slot in impacted:
            ...
            # Dropdown for available teachers (with blank option always present)
            # Show all teachers except the absent one and those already busy at this period (case-insensitive)
            busy_teachers = set()
            for c2 in classes:
                for s2 in sections:
                    tval = self.entries[(c2, s2, day, p)][1].get()
                    if tval:
                        busy_teachers.add(tval.strip().lower())
            available_teachers = [t for t in self.config['teachers'] if t.strip().lower() != teacher_lower and t.strip().lower() not in busy_teachers]
            # Always add blank at the top
        ...
        def save_changes():
            ...
            for slot, tvar, cb in teacher_entries:
                ...
                # Allow blank (no teacher) assignment
                if new_teacher:
                    # Check if teacher is already assigned at this period in any class/section (case-insensitive)
                    for c2 in classes:
                        for s2 in sections:
                            if (c2, s2, day, p) == (class_, section, day, p):
                                continue
                            tval = self.entries[(c2, s2, day, p)][1].get()
                            if tval.strip().lower() == new_teacher.strip().lower():
    ...
    def setup_ui(self):
        ...
        now = datetime.now()
        month_label = ttk.Label(header_frame, text=now.strftime('%B'), font=("Segoe UI", 12, "bold"), background="#2d6cdf", foreground="#fff", anchor="w")
        month_label.pack(side='left', padx=(10, 0), pady=(0, 10))

        header_label = ttk.Label(header_frame, text="Class Flow", font=("Segoe UI", 22, "bold"), background="#2d6cdf", foreground="#fff", anchor="center")
        header_label.pack(side='left', expand=True, fill='x', pady=(0, 0))

        beta_label = ttk.Label(header_frame, text="beta release", font=("Segoe UI", 10, "italic"), background="#2d6cdf", foreground="#fff", anchor="center")
        beta_label.pack(side='left', expand=True, fill='x', pady=(0, 10))

        datetime_label = ttk.Label(header_frame, text=now.strftime('%d %b %Y, %I:%M %p'), font=("Segoe UI", 12), background="#2d6cdf", foreground="#fff", anchor="e")
                ...
                for d in days:
                    for p in range(periods):
                        ...
                        if cell_key in getattr(self, 'impacted_cells', set()):
                            highlight = '#b20000'  # dark red frame
                            entry_bg = '#ff3333'   # strong red
                            entry_fg = 'white'
                            entry_font = ("Segoe UI", 10, "bold")
                            entry_text = '🚫'
                        elif cell_key in getattr(self, 'resolved_cells', set()):
                            highlight = '#ccffcc'  # green
                        frame = tk.Frame(self.grid_frame, bg=highlight if highlight else '#eaf0fa', bd=2, relief="solid")
                        ...
                        teacher_entry = tk.Entry(frame, textvariable=teacher_var, width=12, bg=entry_bg, fg=entry_fg, font=entry_font, relief='solid', bd=2, justify='center')
                        if entry_text:
                            teacher_var.set(entry_text)
                        teacher_entry.pack(side='top', fill='x', padx=1, pady=(0,2))
                        # Track frame for color update
                        self.cell_frames[cell_key] = frame
                        # Track teacher_entry for highlight
                        if not hasattr(self, 'teacher_entries'):
                            self.teacher_entries = {}
                        self.teacher_entries[cell_key] = teacher_entry
                        def on_teacher_change(event=None, key=cell_key):
                            ...
                            self.teacher_entries[key].config(bg='#eaf0fa', fg='black', insertbackground='black', highlightthickness=1, highlightbackground='#eaf0fa')
    ...
    def export_impacted_classes(self, teacher, day):
        ...
        for slot in impacted:
            ...
            ttk.Label(inner, text=f"{class_} {section} Period {p+1}", background="#ffcccc").grid(row=row, column=0, sticky='w', padx=2, pady=2)
            ttk.Label(inner, text=subject).grid(row=row, column=1, sticky='w', padx=2)
            # Dropdown for available teachers (exclude absent teacher)
            ...
            available_teachers = [""] + available_teachers
            # If the current teacher is the absent teacher, default to blank
            current_val = slot['teacher_var'].get()
            tvar = tk.StringVar(value="" if current_val == teacher else current_val)
            cb = ttk.Combobox(inner, textvariable=tvar, values=available_teachers, width=18, state='readonly')
            cb.grid(row=row, column=2, padx=2)
            teacher_entries.append((slot, tvar, cb))
            row += 1
        def save_changes():
            ...
            for slot, tvar, cb in teacher_entries:
                ...
                new_teacher = tvar.get()
                key = (class_, section, day, p)
                slot['teacher_var'].set(new_teacher if new_teacher else "")
                # Also clear the entry widget if blank
                if hasattr(self, 'teacher_cbs') and key in self.teacher_cbs:
                    entry_widget, _ = self.teacher_cbs[key]
                    if not new_teacher:
